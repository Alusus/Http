...
```
عرف مـنادى_اتصال_ويب_سوكت: لقب مؤشر[دالة (اتصال: مؤشر[بـننت.اتـصال], بيانات_المستخدم: مؤشر[فـراغ]): صـحيح];
```
...
```
عرف مـنادى_استعداد_ويب_سوكت: لقب مؤشر[دالة (اتصال: مؤشر[بـننت.اتـصال], بيانات_المستخدم: مؤشر[فـراغ]): فـراغ];
```
...
```
عرف مـنادى_إغلاق_ويب_سوكت: لقب مؤشر[دالة (اتصال: مؤشر[بـننت.اتـصال], بيانات_المستخدم: مؤشر[فـراغ]): فـراغ];
```
...
```
دالة اكتب_في_ويب_سوكت(اتصال: مؤشر[بـننت.اتـصال], رمز_العملية: صـحيح, بيانات: مـؤشر_محارف, طول_البيانات: صـحيح): صـحيح;
```
...
```
دالة اكتب_نصا_في_ويب_سوكت(اتصال: مؤشر[بـننت.اتـصال], بيانات: مـؤشر_محارف, طول_البيانات: صـحيح): صـحيح;
```
...
```
دالة اكتب_ثنائيا_في_ويب_سوكت(اتصال: مؤشر[بـننت.اتـصال], بيانات: مـؤشر_محارف, طول_البيانات: صـحيح): صـحيح;
```
...
```
دالة أغلق_ويب_سوكت(اتصال: مؤشر[بـننت.اتـصال]): صـحيح;
```
...
وحدة مـثال_ويب_سوكت {
    ...
    دالة ابدأ() {
        // بدء تشغيل خادم HTTP
        عرف سياق: مؤشر[بـننت.سـياق] = بـننت.شغل_الخادم(معالج_طلب_بننف~مؤشر، "8080")؛
    ...
    دالة معالج_طلب_بننف(اتصال: مؤشر[بـننت.اتـصال]): صـحيح {
        ...
        بـننت.اطبع(اتصال، "<h1>خادم WebSocket</h1>")؛
        أرجع 1؛
    }

    دالة عند_اتصال_ويب_سوكت(اتصال: مؤشر[بـننت.اتـصال]، بيانات_المستخدم: مؤشر[فـراغ]): صـحيح {
        طـرفية.اطبع("عميل WebSocket متصل\ج")؛
        أرجع 0؛ // قبول الاتصال
    }

    دالة عند_استعداد_ويب_سوكت(اتصال: مؤشر[بـننت.اتـصال]، بيانات_المستخدم: مؤشر[فـراغ]): فـراغ {
        طـرفية.اطبع("WebSocket جاهز\ج")؛
        بـننت.اكتب_نصا_في_ويب_سوكت(اتصال، "أهلاً وسهلاً!")؛
    }
    ...
    دالة عند_بيانات_ويب_سوكت(
        ...
        إذا رمز_العملية == 1 { // إطار نصي
            ...
            // إرجاع الرسالة
            بـننت.اكتب_نصا_في_ويب_سوكت(اتصال، بيانات، طول_البيانات)؛
        }
        أرجع 1؛
    }

    دالة عند_إغلاق_ويب_سوكت(اتصال: مؤشر[بـننت.اتـصال]، بيانات_المستخدم: مؤشر[فـراغ]): فـراغ {
